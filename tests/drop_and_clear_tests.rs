...
fn test_drop_frees_all_memory_multi_level_tree() {
    ...
    
    {
        ...
        assert!(count >= 200, "Should have at least 200 objects, got {}", count);
...
fn test_clear_frees_all_memory() {
    ...
    println!("After insert: {} objects (includes separator key clones)", count_after_insert);
    assert!(count_after_insert >= 100, "Should have at least 100 objects");
...
fn test_clear_and_reuse_with_drop_tracking() {
    ...
    // Clear
    tree.clear();
    assert_eq!(counter.load(Ordering::SeqCst), 0, "First clear leaked memory");
    ...
    // Clear again
    tree.clear();
    assert_eq!(counter.load(Ordering::SeqCst), 0, "Second clear leaked memory");
    ...
    // Final drop
    drop(tree);
    assert_eq!(counter.load(Ordering::SeqCst), 0, "Final drop leaked memory");
}
...
fn test_drop_after_removes() {
    ...
    
    {
        ...
        for i in 10..30 {
            println!("  Removing {} (before: {} objects)", i, counter.load(Ordering::SeqCst));
            let key = DropCounter::new(i, counter.clone());
            ...
            println!("  After remove {}: {} objects (dropped {})", i, after_remove, diff);
        }
        
        // Should have fewer objects now (30 keys + 30 values + some separators)
        let current = counter.load(Ordering::SeqCst);
        println!("After removes: {} objects", current);
        assert!(current < initial_count, "Count should decrease after removes");
...
fn test_simple_remove() {
    ...
    for i in 10..14 {
        ...
        println!("  Removing {} (before: {} objects)...", i, before);
        {
            ...
            println!("    After key create: {}, after remove: {}", after_key_create, after_remove_before_key_drop);
        }
        let after = counter.load(Ordering::SeqCst);
        println!("    After key drop: {} objects (net change: {})", after, (before as i64) - (after as i64));
...
fn test_clear_with_20_items() {
    ...
    println!("Tree has {} leaves, is_leaf_root={}", leaf_count, is_leaf_root);
    ...
    println!("Expected objects (including separators): {}", expected_with_separators);
    
    assert_eq!(after_insert, expected_with_separators, 
        "Should have {} objects (40 leaf items + {} separators)", 
        expected_with_separators, leaf_count - 1);
