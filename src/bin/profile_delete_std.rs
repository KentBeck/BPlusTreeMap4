...
fn main() {
    // Profile delete-heavy workload with std::BTreeMap for comparison
    let n = 1_000_000; // 1M operations for profiling

    println!("Profiling {} delete operations with std::BTreeMap", n);

    // Phase 1: Build the tree
    println!("Phase 1: Building tree with {} items...", n);
    let mut map = BTreeMap::new();

    // Generate random-ish data using LCG (same as profile_delete)
