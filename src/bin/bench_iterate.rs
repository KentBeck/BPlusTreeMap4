...
fn main() {
    ...
    for &n in &sizes {
        ...
        println!("    Forward:  {:.3}s ({:.0} ops/sec)", forward_bp, n as f64 / forward_bp);
        println!("    Backward: {:.3}s ({:.0} ops/sec)", backward_bp, n as f64 / backward_bp);
        ...
        println!("    Forward:  {:.3}s ({:.0} ops/sec)", forward_std, n as f64 / forward_std);
        println!("    Backward: {:.3}s ({:.0} ops/sec)", backward_std, n as f64 / backward_std);
        
        // Comparison
        let forward_ratio = forward_bp / forward_std;
        let backward_ratio = backward_bp / backward_std;
        println!("  Ratio (BPlusTree/std):");
        println!("    Forward:  {:.2}x {}", forward_ratio, 
                 if forward_ratio < 1.0 { "(faster)" } else { "(slower)" });
        println!("    Backward: {:.2}x {}", backward_ratio, 
                 if backward_ratio < 1.0 { "(faster)" } else { "(slower)" });
        println!();
    }
}
